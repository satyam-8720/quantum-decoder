"""
Verilog Testbench Generator for LDPC Decoder Output Printing
------------------------------------------------------------

This script programmatically generates a Verilog testbench
(tb_decoder_printer.v) used to verify and inspect the output
of the LDPC decoder in simulation.

What this file does:
1. Creates a Verilog testbench that instantiates the LDPC decoder
   with 576 signed LLR inputs and 576 decoded output bits.
2. Loads LLR values from an external memory file
   (llr_input_qutip.mem) generated by Python/QuTiP simulations.
3. Applies clock and reset stimulus to the decoder.
4. Waits for the decoder to complete processing.
5. Prints the full decoded codeword (P1–P576) to the simulation
   console in a Python-compatible array format.
6. Allows easy copy-paste of decoder outputs back into Python
   for validation, plotting, or comparison with ground truth.

Purpose:
This script acts as a bridge between Python-based LLR generation
and Verilog-based LDPC decoder verification, enabling fast
end-to-end debugging without manual testbench editing.
